...
export default function App() {
  ...
  // NOTE: loaded starts true immediately — localStorage data is shown
  // right away, and Supabase data updates in the background.
  const {
  ...
  // ── Portfolio logic wired to persistence setters ───────
  const {
  ...
  const allTickers = useMemo(() => {
    ...
    if (["marketwatch","macro","fx","commodities","screener"].includes(activeTab))
  ...
  // ── Merge live quotes onto enriched holdings ───────────
  const liveHoldings = useMemo(() => {
    return enrichedHoldings.map(h => {
      ...
      return {
        ...h,
        livePrice: q.price, liveChange: q.change, liveChangePct: q.changePct,
        livePositive: q.positive, liveName: q.name,
        liveMarketCap: q.marketCap, liveVolume: q.volume,
  ...
  return (
    <div style={{ minHeight:"100vh" }}>
      ...
      <main style={{ maxWidth:1320, margin:"0 auto", padding:"24px 24px 60px" }}>
        {activeTab === "builder"     && (
          <PortfolioBuilder
            holdings={liveHoldings} totalWeight={totalWeight} metrics={metrics}
            addStock={addStock} removeStock={removeStock} updateWeight={updateWeight}
            equalWeight={equalWeight} addCustomStock={addCustomStock}
        ...
        {activeTab === "screener"    && <Screener stockData={stockData} holdings={liveHoldings} onAddToPortfolio={addStock} />}
        {activeTab === "marketwatch" && (
          <MarketWatch
            holdings={liveHoldings} stockData={stockData}
            watchlistTickers={watchlistTickers} setWatchlistTickers={setWatchlistTickers}
